...
for idx, (X, y) in tqdm(enumerate(test_dataloader)):
    if inputs is None:
        ...
        inputs = torch.cat((inputs, X.detach().cpu()), dim=0)
    logits = model(X)
...
pickle.dump(inputs, open(f'test_inputs_{args.study}_{args.treat}_{args.study_run}.pkl', 'wb'))
pickle.dump(targets, open(f'test_targets_{args.study}_{args.treat}_{args.study_run}.pkl', 'wb'))
...
pickle.dump(inputs, open(f'train_inputs_{args.study}_{args.treat}_{args.study_run}.pkl', 'wb'))
pickle.dump(targets, open(f'train_targets_{args.study}_{args.treat}_{args.study_run}.pkl', 'wb'))
exit()
