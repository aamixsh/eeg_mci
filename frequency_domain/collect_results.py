...
for run in range(args.runs):
    ...
    train_pats, test_pats = get_train_test_pats_from_run(f'{args.results_dir}_{args.rm_ch_str}/{args.study}_{args.treat}_{args.contig_len}_{args.norm_type}_{args.balanced}_{args.car_str}.txt', run)
    ...
    file = open(f'{args.results_dir}_{args.rm_ch_str}/{args.study}_{args.treat}_{args.contig_len}_{args.norm_type}_{args.balanced}_{args.car_str}_new.txt', 'a')
    ...
    model.load_state_dict(torch.load(f'{args.weights_dir}_{args.rm_ch_str}/{args.study}_{args.treat}_{args.contig_len}_{args.norm_type}_{args.balanced}_{args.car_str}_{run}.pth'))
    ...
    # auroc_micro = MulticlassAUROC(num_classes=2, average='micro')
    # auroc_micro.update(predictions, targets)
    # auprc_micro = MulticlassAUPRC(num_classes=2, average='micro')
    # auprc_micro.update(predictions, targets)
    auroc_none = MulticlassAUROC(num_classes=2, average=None)
    auroc_none.update(predictions, targets)
    auprc_none = MulticlassAUPRC(num_classes=2, average=None)
    auprc_none.update(predictions, targets)
    ...
    print (f'Test AUPRC (macro): {auprc.compute().item()}')
    file.write(f'Test AUPRC (macro): {auprc.compute().item()}\n')
    print (f'Test AUROC (macro): {auroc.compute().item()}')
    file.write(f'Test AUROC (macro): {auroc.compute().item()}\n')
    # print (f'Test AUPRC (micro): {auprc_micro.compute().item()}')
    # file.write(f'Test AUPRC (micro): {auprc_micro.compute().item()}\n')
    # print (f'Test AUROC (micro): {auroc_micro.compute().item()}')
    # file.write(f'Test AUROC (micro): {auroc_micro.compute().item()}\n')
    auprc_none_vals = auprc_none.compute()
    print (f'Individual class test AUPRCs: "0" - {auprc_none_vals[0].item()}, "1" - {auprc_none_vals[1].item()}')
    file.write(f'Individual class test AUPRCs: "0" - {auprc_none_vals[0].item()}, "1" - {auprc_none_vals[1].item()}\n')
    auroc_none_vals = auroc_none.compute()
    print (f'Individual class test AUROCs: "0" - {auroc_none_vals[0].item()}, "1" - {auroc_none_vals[1].item()}')
    file.write(f'Individual class test AUROCs: "0" - {auroc_none_vals[0].item()}, "1" - {auroc_none_vals[1].item()}\n')
    ...
    for i, (X, y) in tqdm(enumerate(temp_loader)):
