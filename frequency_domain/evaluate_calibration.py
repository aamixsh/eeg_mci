...
for contig_len in contig_lens:
    ...
    for norm in norms:
        for balance in balances:
            ...
            print (pats)
            file = open(f'results_split_14_15_samesplit/mci_ctrlb_{contig_len}_{norm}_{balance}_calibration.txt', 'a')
            for run in range(args.runs):

                train_pats, test_pats = get_train_test_pats_from_run(f'{args.results_dir}_{args.rm_ch_str}_samesplit/{args.study}_{args.treat}_{args.contig_len}_{args.norm_type}_{args.balanced}.txt', run)
...
#                 test_weights = weights[pred0, pred1]
                ...
                # Load weights
                model.load_state_dict(torch.load(f'{args.weights_dir}_{args.rm_ch_str}_samesplit/{args.study}_{args.treat}_{args.contig_len}_{args.norm_type}_{args.balanced}_{run}.pth'))
