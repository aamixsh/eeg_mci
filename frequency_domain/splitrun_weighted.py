...
parser.add_argument('--gmm_components', type=int, default=7)
...
for run in range(args.runs):
    ...
    file = open(f'{args.results_dir}_{args.rm_ch_str}/{args.study}_{args.treat}_{args.contig_len}_{args.norm_type}_{args.balanced}_trainweighted_{args.gmm_components}.txt', 'a')
...
#     # Weighted on scores
#     test_weights = np.abs(gm0.score_samples(testX) - gm1.score_samples(testX))
    ...
    for epoch in pbar:
        ...
        for X, y in train_dataloader:
            ...
            npX = X.reshape((-1, X.shape[1] * X.shape[2])).cpu().numpy()
            # weighted on mode distance
            pred0, pred1 = gm0.predict(npX), gm1.predict(npX)

            weight = torch.tensor(weights[pred0, pred1]).cuda()
            
#             # weighted on score
#             weight = torch.tensor(np.abs(gm0.score_samples(npX) - gm1.score_samples(npX))).cuda()

            loss = nll(logits, y, reduction='none')
            loss = (loss * weight).mean() / weight.sum()
    ...
    torch.save(model.state_dict(), f'{args.weights_dir}_{args.rm_ch_str}/{args.study}_{args.treat}_{args.contig_len}_{args.norm_type}_{args.balanced}_trainweighted_{args.gmm_components}_{run}.pth')
