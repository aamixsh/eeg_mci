...
parser.add_argument('--meta_dir', type=str,default='meta_data_evt')
parser.add_argument('--weights_dir', type=str,default='weights_spectral_evt')
parser.add_argument('--results_dir', type=str,default='results_spectral_evt')
parser.add_argument('--data_path', type=str,default='./mci_ctrl_ctrlb_psd_evt.pkl')
parser.add_argument('--mask_path', type=str,default='./mask_evt.npy')
...
parser.add_argument('--treat', type=str, default='ctrl')
        ...
        for start_evt in start_evts:
            for filter_type in filter_types:
                for shuffle in shuffles:
                    ...
                    file = open(f'{args.results_dir}/{args.study}_{args.treat}_{args.remove_channels}_{args.start_evt}{balanced}.txt', 'a')
                    ...
                    if len(channels) == 3:
                        ...
                        cnn = MLPS(num_channels=len(channels), num_psd=args.num_psd, out_dim=args.num_classes)
#                         cnn = MLP(in_dim=in_dim, out_dim=args.num_classes)
                    if args.cuda:
                        cnn = cnn.cuda()
                    ...
                    torch.save(cnn.state_dict(), f'{args.weights_dir}/{args.study}_{args.treat}_{args.remove_channels}_{args.start_evt}{balanced}_{run}_split.pth')
...
#                         cnn.load_state_dict(torch.load(f'{args.weights_dir}/{args.study}_{args.remove_channels}_{args.start_evt}_{args.contig_len}_{args.num_epochs}_{args.filter_type}_{args.shuffle}_{args.normalize}_{args.lr}.pth'))
                    ...
                    temp_dataset = EEGSDataset(data_dict=data, locs=test_locs, study=args.study, treat=args.treat, norms=n, evt=args.include_evt)
