...
parser.add_argument('--meta_dir', type=str,default='meta_data_14_15_notevt')
parser.add_argument('--weights_dir', type=str,default='weights_spectral_14_15_notevt')
parser.add_argument('--results_dir', type=str,default='results_spectral_14_15_notevt')
parser.add_argument('--data_path', type=str,default='./mci_ctrl_ctrlb_psd_14_15_notevt.pkl')
parser.add_argument('--mask_path', type=str,default='./mask_14_15_notevt.npy')
...
parser.add_argument('--study_run', type=int, default=4)
...
if args.cnn:
    args.results_dir = 'cnn_' + args.results_dir
    args.weights_dir = 'cnn_' + args.weights_dir
...
for run in range(args.runs):
    ...
    file = open(f'{args.results_dir}/{args.study}_{args.treat}_{args.remove_channels}_{args.start_evt}{safe_str}_interesting.txt', 'a')
    ...
    train_pats, test_pats = get_train_test_pats_from_run(f'{args.results_dir}/{args.study}_{args.treat}_{args.remove_channels}_{args.start_evt}{safe_str}.txt', args.study_run)
    pats = {'train': train_pats, 'test': test_pats}
    
    data = all_data
    # Get clean train_test locations
    train_locs, test_locs = get_spectral_train_test_locs_from_pats(args, data, locs, pats)
...
#     norms = {'mins': torch.tensor(channel_mins).type(torch.float32), 'maxs': torch.tensor(channel_maxs).type(torch.float32)}
    ...
    if args.cnn:
        model = CNNS(num_channels=len(channels), num_psd=args.num_psd, kernel=args.kernel, out_dim=args.num_classes)
    else:
        model = MLPS(num_channels=len(channels), num_psd=args.num_psd, out_dim=args.num_classes)
        
    model.load_state_dict(torch.load(f'{args.weights_dir}/{args.study}_{args.treat}_{args.remove_channels}_{args.start_evt}{safe_str}_{args.study_run}_split.pth'))
