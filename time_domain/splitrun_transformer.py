...
from models import CNN, Transformer, TransformerS
from data import EEGDataset
from utils import *

parser = argparse.ArgumentParser(description='Run EEG Raw experiments (tf)')
...
parser.add_argument('--results_dir', type=str,default='./results_tf_split')
parser.add_argument('--weights_dir', type=str,default='./weights_tf_split')
...
# conf_spectral = read_grid_confs(conf_path)[0]
...
for run in range(args.runs):
    ...
    file = open(f'{args.results_dir}_{args.rm_ch_str}/{args.study}_{args.treat}_{args.contig_len}_{args.norm_type}_{args.balanced}.txt', 'a')
    ...
    if args.model_type == 'transformer':
        model = Transformer(num_channels=len(channels), 
                        seq_len=(args.contig_len  - args.window_len) // args.window_stride + 1, 
                        stride_len = args.window_stride, 
                        d_model=args.window_len, 
                        nhead=args.window_len // 2, 
                        n_classes=args.num_classes, 
                        verbose=False, cuda=args.cuda)
    else:
        model = TransformerS(num_channels=len(channels), 
                        seq_len=args.contig_len, 
                        stride_len = args.window_stride, 
                        d_model=len(channels), 
                        nhead=1, 
                        dense_len=128,
                        n_classes=args.num_classes, 
                        verbose=False, cuda=args.cuda)
    ...
    torch.save(model.state_dict(), f'{args.weights_dir}_{args.rm_ch_str}/{args.study}_{args.treat}_{args.contig_len}_{args.norm_type}_{args.balanced}_{run}.pth')
