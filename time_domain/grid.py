...
parser = argparse.ArgumentParser(description='Run EEG Raw Grid Search')
...
parser.add_argument('--results_dir', type=str,default='./gs_results')
parser.add_argument('--data_path', type=str,default='./data/mci_ctrlb')
parser.add_argument('--sf', type=float, default=250.)
parser.add_argument('--contig_len', type=int, default=500)
...
parser.add_argument('--lrs', type=str, default='1e-4_1e-5')
parser.add_argument('--batch_sizes', type=str, default='16_32')
parser.add_argument('--model_confs_path', type=str, default='./grid_confs/conf')
...
lrs = [float(i) for i in args.lrs.split('_')]
batch_sizes = [int(i) for i in args.batch_sizes.split('_')]

conf_path = args.model_confs_path + '_' + args.model_type + '_' + str(args.contig_len) + '_' + str(len(channels)) + '.txt'
confs = read_grid_confs(conf_path)
...
for run in range(args.runs):
    # Get clean locations
    locs = get_clean_spectral_locs(args, all_data, typs)
    ...
    train_locs, test_locs = get_spectral_train_test_locs(args, data, locs)
    ...
    for lr in lrs:
        for batch_size in batch_sizes:
            for conf in confs:
                print (lr, batch_size, conf)

                file = open(f'{args.results_dir}_{args.rm_ch_str}/{args.study}_{args.treat}_{args.contig_len}_{args.norm_type}_{args.balanced}_{lr}_{batch_size}_{conf[0]}.txt', 'a')
